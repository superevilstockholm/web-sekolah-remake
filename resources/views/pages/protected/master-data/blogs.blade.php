...
@section('title', 'Blogs - Master Data - Al-Layyinah')
@section('description',
...
@section('content')
    <div class="container">
        <div class="row mb-3 justify-content-md-between align-items-md-center">
            <div class="col-md-6 col-12">
                <h3 class="page-title mb-2 mb-md-1 fw-semibold">Blogs Data</h3>
            ...
            <div class="col-lg-2 col-md-3 col-12">
                <div class="d-grid">
                    <button class="btn btn-primary h-100" data-bs-toggle="modal" data-bs-target="#blogsCreateModal">
                        Create</button>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade" id="blogsShowModal" tabindex="-1" aria-labelledby="blogsShowModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content border-0 shadow">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title" id="blogsShowModalLabel">
                        <i class="la la-eye me-2"></i>Detail Blogs
                    </h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"
                        aria-label="Close"></button>
                </div>
                <div class="modal-body p-0 m-0">
                    <div id="blogsDetailContent">
    ...
    <div class="modal fade" id="blogsCreateModal" tabindex="-1" aria-labelledby="blogsCreateModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content border-0 shadow">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title" id="blogsCreateModalLabel">
                        <i class="la la-plus-circle me-2"></i>Tambah Blogs
                    </h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"
                        aria-label="Close"></button>
                </div>
                <form id="blogsCreateForm" enctype="multipart/form-data">
    ...
    <div class="modal fade" id="blogsEditModal" tabindex="-1" aria-labelledby="blogsEditModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content border-0 shadow">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title" id="blogsEditModalLabel">
                        <i class="la la-edit me-2"></i>Edit Blogs
                    </h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"
                        aria-label="Close"></button>
                </div>
                <form id="blogsEditForm" enctype="multipart/form-data">
    ...
    <script>
        const baseUrl = "{{ url('/api/master-data/blogs') }}";
        let fetchBlogs;
        document.addEventListener('DOMContentLoaded', function() {
            ...
            fetchBlogs = async function(page = 1) {
                blogsTable.innerHTML = `<tr><td colspan="7" class="text-center">Loading...</td></tr>`;
                try {
                    let params = {
                        page,
                    ...
                    blogsTable.innerHTML = `
                        <tr><td colspan="7" class="text-center text-danger">Gagal memuat data blog</td></tr>
                    `;
                }
            };
            function renderBlogs(data) {
                ...
                blogsTable.innerHTML = blogs.map((item, index) => `
                    <tr>
                        ...
                        <td>${item.slug}</td>
                        <td>${item.category === 'blog' ? 'Blog' : 'Artikel'}</td>
                        <td class="d-flex align-items-center gap-1 justify-content-end">
                            <button class="btn btn-sm btn-outline-success" onclick="showBlogsDetail(${item.id})">
                                <i class="la la-eye"></i>
                            </button>
                            <button class="btn btn-sm btn-outline-primary" onclick="showBlogsEdit(${item.id})">
                                <i class="la la-edit"></i>
                            </button>
                            <button class="btn btn-sm btn-outline-danger" onclick="deleteBlogs(${item.id})">
                ...
                document.querySelectorAll('#pagination a[data-page]').forEach(link => {
                    link.addEventListener('click', e => {
                        ...
                        if (page && page !== '#') fetchBlogs(page);
                    });
                });
            }
            searchButton.addEventListener('click', () => fetchBlogs(1));
            limitSelect.addEventListener('change', () => fetchBlogs(1));
            fetchBlogs();
        });
        async function showBlogsDetail(id) {
            const blogsDetailContent = document.getElementById('blogsDetailContent');
            blogsDetailContent.innerHTML = `<p class="text-center text-muted mb-0">Memuat data...</p>`;
            const modal = new bootstrap.Modal(document.getElementById('blogsShowModal'));
            modal.show();
            try {
                ...
                if (!status) throw new Error(message || 'Gagal memuat data blog.');
                        ...
                        <div class="col-12 position-relative mb-2 mb-md-3" style="height: 250px;">
                            <img src="${blogsItem.image_url}"
                                ...
                                <div class="container h-100 d-flex align-items-end justify-content-start">
                                    <h1 class="px-3 text-primary">${blogsItem.title}</h1>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="container">
                        <div class="row px-1 py-3 px-md-3">
                            <div class="col-6 mb-2">
                                ...
                                <p class="text-muted mb-0">${blogsItem.category === 'blog' ? 'Blog' : 'Artikel'}</p>
                            </div>
                            <div class="col-6 mb-2">
                                <strong><i class="la la-key me-1 text-primary"></i>Author:</strong>
                                <p class="text-muted mb-0">${blogsItem.user?.name || 'Tidak diketahui'}</p>
                            </div>
                            <div class="col-6 mb-2">
                                <strong><i class="la la-key me-1 text-primary"></i>ID:</strong>
                                <p class="text-muted mb-0">${blogsItem.id}</p>
                            </div>
                            <div class="col-6 mb-2">
                                <strong><i class="la la-link me-1 text-primary"></i>Slug:</strong>
                                <p class="text-muted mb-0">${blogsItem.slug}</p>
                            </div>
                            <div class="col-6 mb-2">
                                <strong><i class="la la-calendar me-1 text-primary"></i>Dibuat:</strong>
                                <p class="text-muted mb-0">${formatDate(blogsItem.created_at)}</p>
                            </div>
                            <div class="col-6 mb-2">
                                <strong><i class="la la-refresh me-1 text-primary"></i>Diperbarui:</strong>
                                <p class="text-muted mb-0">${formatDate(blogsItem.updated_at)}</p>
                            </div>
                            <div class="col-12">
                                ...
                                <div class="mt-1"
                                    ...
                                    ${blogsItem.content ? marked.parse(blogsItem.content) : '<em>Tidak ada konten.</em>'}
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                blogsDetailContent.innerHTML = `
                    <div class="alert alert-danger text-center mb-0">
                        <i class="la la-exclamation-circle me-1"></i> Gagal memuat detail blog.
                    </div>
                `;
            }
        }
            ...
            upload() {
                return this.loader.file
                    .then(file => new Promise((resolve, reject) => {
                        ...
                        data.append('directory', 'blogs'); // contoh: upload ke folder "blogs"
        ...
        document.getElementById('blogsCreateForm').addEventListener('submit', async function(e) {
            e.preventDefault();
            ...
            try {
                ...
                const res = response.data;
                if (!res.status) throw new Error(res.message || 'Gagal membuat blog');
                Swal.fire({
                    icon: 'success',
                    title: 'Berhasil',
                    text: res.message || 'Blogs berhasil dibuat'
                });
                setTimeout(() => {
                    bootstrap.Modal.getInstance(document.getElementById('blogsCreateModal')).hide();
                    form.reset();
                    createImagePreview.src = "{{ asset('static/img/no-image-placeholder.svg') }}";
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Gagal',
                    text: error.response?.data?.message || 'Terjadi kesalahan saat membuat blog.'
                });
            }
        });
        // Reset modal saat ditutup
        const blogsCreateModal = document.getElementById('blogsCreateModal');
        blogsCreateModal.addEventListener('hidden.bs.modal', function() {
            createImagePreview.src = "{{ asset('static/img/no-image-placeholder.svg') }}";
            document.getElementById('blogsCreateForm').reset();
            if (typeof createEditor !== 'undefined') createEditor.setData('');
        });
        // ----------- Edit -----------
        let editEditor;
        ...
        async function showBlogsEdit(id) {
            try {
                const response = await axios.get(`${baseUrl}/${id}`, {
                    headers: {
                        'Accept': 'application/json',
                ...
                const res = response.data;
                if (!res.status) throw new Error(res.message || 'Gagal memuat data blog');
                const blogs = res.data;
                document.getElementById('editId').value = blogs.id;
                document.getElementById('editTitle').value = blogs.title;
                document.getElementById('editCategory').value = blogs.category;
                editImagePreview.src = blogs.image_url || "{{ asset('static/img/no-image-placeholder.svg') }}";
                // Tunggu editor siap sebelum setData
                const editor = await editEditorPromise;
                editor.setData(blogs.content || '');
                const modal = new bootstrap.Modal(document.getElementById('blogsEditModal'));
                modal.show();
            } catch (error) {
                console.error(error);
                Swal.fire({
                    icon: 'error',
                    title: 'Gagal',
                    text: error.response?.data?.message || 'Tidak dapat memuat data blog.'
                });
            }
        }
        // Submit edit form
        document.getElementById('blogsEditForm').addEventListener('submit', async function(e) {
            e.preventDefault();
            ...
            try {
                ...
                const res = response.data;
                if (!res.status) throw new Error(res.message || 'Gagal memperbarui blog');
                Swal.fire({
                    icon: 'success',
                    title: 'Berhasil',
                    text: res.message || 'Blogs berhasil diperbarui'
                });
                setTimeout(() => {
                    bootstrap.Modal.getInstance(document.getElementById('blogsEditModal')).hide();
                    form.reset();
                    editImagePreview.src = "{{ asset('static/img/no-image-placeholder.svg') }}";
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Gagal',
                    text: error.response?.data?.message || 'Terjadi kesalahan saat memperbarui blog.'
                });
            }
        });
        // Reset modal saat ditutup
        document.getElementById('blogsEditModal').addEventListener('hidden.bs.modal', function() {
            editImagePreview.src = "{{ asset('static/img/no-image-placeholder.svg') }}";
            document.getElementById('blogsEditForm').reset();
            if (typeof editEditor !== 'undefined') editEditor.setData('');
        });
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    try {
                        ...
                        const res = response.data;
                        if (!res.status) throw new Error(res.message || 'Gagal menghapus blog');
                        Swal.fire({
                            icon: 'success',
                            title: 'Berhasil',
                            text: res.message || 'Blogs berhasil dihapus'
                        });
