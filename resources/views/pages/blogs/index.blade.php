...
@section('content')
    ...
    <section class="mt-3 mb-5 faq-section">
        <div class="container">
            <h1 class="faq-title text-center mb-5 py-2 text-black fw-normal">Blogs & Artikel</h1>
            <div class="row align-items-center">
                ...
                <div class="col-12 col-md-6 col-lg-4">
                    <p class="fw-normal mb-3">Telusuri artikel dan blog terbaru kami. Dari tips pendidikan hingga cerita menarik, temukan semua informasi terbaru di Al-Layyinah.</p>
                </div>
                <div class="col-lg-1 d-none d-lg-block"></div>
            </div>
        </div>
                ...
                <div class="col-lg-4 col-12 mb-4 mb-lg-0">
                    <h5 class="text-primary d-flex align-items-center gap-2 mb-3" style="font-size: 0.9rem;">
                        <img height="20" src="https://sph.edu/wp-content/uploads/2021/08/SPH-Rectangle.png"
                            alt="">Blogs Al-Layyinah
                    </h5>
                    <h3 class="faq-title text-black"><span class="faq-title text-primary">Blog dan Artikel</span> Terbaru</h3>
                </div>
    ...
    <script>
        ...
        async function fetchAcara(page = 1) {
            try {
                const res = await fetch(`/api/master-data/blogs?limit=${perPage}&page=${page}`);
                const json = await res.json();
                if (!json.status) throw new Error(json.message || 'Error fetching blogs');
        ...
        async function renderAcara(page = 1) {
            ...
            if (!data) {
                container.innerHTML = "<p class='text-danger'>Gagal memuat blogs</p>";
                return;
            }
            container.innerHTML = "";
            data.data.forEach(blogs => {
                ...
                col.innerHTML = `
                    <a href="/blogs/${blogs.slug}" class="text-decoration-none">
                        <div class="card h-100 shadow-sm border-0 position-relative rounded">
                            ...
                            <div class="card-img-overlay d-flex flex-column justify-content-end p-2"
                                ...
                                <span class="mb-1 text-uppercase text-light" style="font-size:0.7rem;">${blogs.category}</span>
                                <h6 class="card-title text-white mb-0">${blogs.title}</h6>
                            </div>
                        </div>
                    </a>
                `;
